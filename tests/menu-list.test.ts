...
describe("shopctl menu list", () => {
  ...
  test("table output shows indented item hierarchy", async () => {
    ...
    const homeLine = lines.find((l: string) => l.includes("Home") && l.includes("HTTP"));
    const aboutLine = lines.find((l: string) => l.includes("About") && l.includes("HTTP") && !l.includes("Home"));
    const teamLine = lines.find((l: string) => l.includes("Team") && l.includes("HTTP"));
