        ...
        if (searchBehavior === "multiple") {
          return new Response(JSON.stringify({
            data: {
              products: {
                edges: [
                  { node: { id: "gid://shopify/Product/5001", title: "Doomed Widget", status: "ACTIVE" } },
                  { node: { id: "gid://shopify/Product/5002", title: "Doomed Widget Pro", status: "DRAFT" } },
                ],
              },
            },
          }), { headers: { "Content-Type": "application/json" } });
...
describe("shopctl product delete — dry run (no --yes)", () => {
  ...
  test("dry run with --json outputs { data } envelope", async () => {
    const { stdout, exitCode } = await run(["product", "delete", "5001", "--json"]);
    expect(exitCode).toBe(0);
...
describe("shopctl product delete — with --yes", () => {
  ...
  test("--yes --json returns { data } envelope with deleted info", async () => {
    const { stdout, exitCode } = await run(["product", "delete", "5001", "--yes", "--json"]);
    expect(exitCode).toBe(0);
    const parsed = JSON.parse(stdout);
    expect(parsed.data.id).toBe("gid://shopify/Product/5001");
    expect(parsed.data.title).toBe("Doomed Widget");
...
describe("shopctl product delete — ID resolution", () => {
  ...
  test("accepts full GID", async () => {
    const { exitCode } = await run(["product", "delete", "gid://shopify/Product/5001", "--yes"]);
    expect(exitCode).toBe(0);
    expect(deleteCalled).toBe(true);
  });
  ...
  test("accepts title string", async () => {
    searchBehavior = "single";
    const { exitCode } = await run(["product", "delete", "Doomed Widget", "--yes"]);
    expect(exitCode).toBe(0);
    expect(deleteCalled).toBe(true);
  });
});
...
describe("shopctl product delete — errors", () => {
  ...
  test("multiple title matches exits 1 with candidates", async () => {
    ...
    const { stdout, exitCode } = await run(["product", "delete", "Doomed Widget"]);
    expect(exitCode).toBe(1);
    expect(deleteCalled).toBe(false);
  });
  ...
  test("API error on delete exits 1", async () => {
    ...
    const { stderr, exitCode } = await run(["product", "delete", "5001", "--yes"]);
    expect(exitCode).toBe(1);
