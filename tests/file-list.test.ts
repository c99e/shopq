	...
	mockServer = Bun.serve({
		...
		fetch: async (req) => {
			...
			if (query.includes("files")) {
				...
				// Type filtering
				if (variables?.query?.includes("media_type:")) {
					const typeMatch = variables.query.match(/media_type:(\w+)/);
					const filterType = typeMatch?.[1];
					const _typeMap: Record<string, string> = {
						IMAGE: "IMAGE",
						VIDEO: "VIDEO",
						GENERIC_FILE: "application/pdf",
					};
					const filtered = MOCK_FILES.filter(
						(f) =>
							(f.node as any).mediaContentType === filterType ||
							(filterType === "GENERIC_FILE" &&
								(f.node as any).mimeType != null),
					);
					return new Response(
						JSON.stringify(
							makeFilesResponse(filtered, false, "cursor-filtered"),
						),
						{ headers: { "Content-Type": "application/json" } },
					);
				}
