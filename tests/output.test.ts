...
import { type Column, formatError, formatOutput } from "../src/output";
...
describe("formatOutput — JSON mode", () => {
	...
	test("includes pageInfo when provided", () => {
		...
		const { stdout } = captureOutput(() => {
			formatOutput(sampleData, columns, {
				json: true,
				noColor: true,
				pageInfo,
			});
	...
	test("renders single object data", () => {
		const { stdout } = captureOutput(() => {
			formatOutput({ id: "1", title: "Widget" }, columns, {
				json: true,
				noColor: true,
			});
		});
...
describe("formatOutput — table mode", () => {
	...
	test("shows pagination hint when hasNextPage is true", () => {
		...
		expect(stdout).toContain(
			"More results available. Use --cursor cursor_xyz to see next page.",
		);
	});
	...
	test("renders single object as key-value pairs", () => {
		const { stdout } = captureOutput(() => {
			formatOutput({ id: "1", title: "Widget", status: "active" }, columns, {
				json: false,
				noColor: true,
			});
		});
