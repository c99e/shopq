...
const MOCK_COLLECTION_NO_IMAGE = {
  ...MOCK_COLLECTION,
...
describe("misty collection get", () => {
  ...
  test("lookup by full GID returns collection", async () => {
    const { stdout, exitCode } = await run(["collection", "get", "gid://shopify/Collection/123456"]);
    expect(stdout).toContain("Summer Sale");
    expect(exitCode).toBe(0);
  });

  test("lookup by handle returns collection", async () => {
    const { stdout, exitCode } = await run(["collection", "get", "summer-sale"]);
    expect(stdout).toContain("Summer Sale");
    expect(exitCode).toBe(0);
  });
  ...
  test("--json returns data envelope with all fields", async () => {
    const { stdout, exitCode } = await run(["collection", "get", "123456", "--json"]);
  ...
  test("--json returns productsCount as object with count and precision", async () => {
    ...
    const parsed = JSON.parse(stdout);
    expect(parsed.data.productsCount).toEqual({ count: 15, precision: "EXACT" });
  });

  test("--json returns image as object", async () => {
    const { stdout } = await run(["collection", "get", "123456", "--json"]);
    const parsed = JSON.parse(stdout);
    expect(parsed.data.image).toEqual({ url: "https://cdn.shopify.com/summer.jpg", alt: "Summer collection" });
  });

  test("--json returns seo as object", async () => {
    const { stdout } = await run(["collection", "get", "123456", "--json"]);
    const parsed = JSON.parse(stdout);
    expect(parsed.data.seo).toEqual({ title: "Summer Sale | Test Store", description: "Shop our summer collection" });
  });
  ...
  test("not found by handle exits with code 1 and stderr message", async () => {
    const { stderr, exitCode } = await run(["collection", "get", "nonexistent"]);
    expect(stderr).toContain("not found");
    expect(exitCode).toBe(1);
  });
