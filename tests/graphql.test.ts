...
describe("resolveConfig", () => {
	...
	afterEach(() => {
		...
		if (originalEnv.SHOPIFY_STORE === undefined)
			delete process.env.SHOPIFY_STORE;
		if (originalEnv.SHOPIFY_CLIENT_ID === undefined)
			delete process.env.SHOPIFY_CLIENT_ID;
		if (originalEnv.SHOPIFY_CLIENT_SECRET === undefined)
			delete process.env.SHOPIFY_CLIENT_SECRET;
	});
...
describe("createClient", () => {
	...
	test("exchanges credentials and sends access token in header", async () => {
		...
		const srv = startServer(async (req) => {
			...
			return new Response(
				JSON.stringify({ data: { shop: { name: "Test" } } }),
				{
					headers: { "Content-Type": "application/json" },
				},
			);
		...
		expect(graphqlHeaders!.get("X-Shopify-Access-Token")).toBe(
			"exchanged-token-123",
		);
	});
	...
	test("sends parameterized variables in request body", async () => {
		...
		expect(receivedBody.query).toBe(
	...
	test("returns data on success", async () => {
		const srv = startServer(async (req) => {
			...
			return new Response(
				JSON.stringify({ data: { shop: { name: "TestShop" } } }),
				{
					headers: { "Content-Type": "application/json" },
				},
			);
	...
	test("throws on GraphQL-level errors", async () => {
		...
		expect(client.query('{ product(id: "1") { title } }')).rejects.toThrow(
			"Product not found",
		);
	});
