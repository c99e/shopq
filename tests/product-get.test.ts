...
const FULL_PRODUCT = {
  ...
  vendor: "Acme",
  tags: ["sale", "new"],
  descriptionHtml: "<p>A wonderful widget that does many things and has a very long description that should be truncated in table mode</p>",
  variants: {
    edges: [
      {
        node: {
...
beforeAll(() => {
  mockServer = Bun.serve({
    ...
    fetch: async (req) => {
      ...
      if (query.includes("ProductGet") || (query.includes("product(id:") && !query.includes("products("))) {
        if (mockBehavior === "none") {
          return new Response(JSON.stringify({ data: { product: null } }), {
            headers: { "Content-Type": "application/json" },
          });
        }
        return new Response(JSON.stringify({ data: { product: FULL_PRODUCT } }), {
          headers: { "Content-Type": "application/json" },
        });
      }
        ...
        if (mockBehavior === "multiple") {
          return new Response(JSON.stringify({
            data: {
              products: {
                edges: [
                  { node: { id: FULL_PRODUCT.id, title: FULL_PRODUCT.title, status: FULL_PRODUCT.status } },
                  { node: { id: PRODUCT_B.id, title: PRODUCT_B.title, status: PRODUCT_B.status } },
                ],
              },
            },
          }), {
...
describe("ID resolution", () => {
  ...
  test("non-numeric string triggers title search", async () => {
    ...
    const searchReq = allRequests.find((r) => r.query.includes("ProductSearch"));
...
describe("misty product get — by ID", () => {
  ...
  test("--json returns full product in { data } envelope", async () => {
    mockBehavior = "single";
    const { stdout, exitCode } = await run(["product", "get", "1001", "--json"]);
